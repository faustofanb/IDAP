...
- 实施计划（plan）、需求规格（spec）、任务清单（tasks）与契约（contracts）等专题文档视为“专题规格”，在信息架构上归属 `specs/`；本汇编仅做阅读聚合，不替代原文件。
...
- 前端：Vue 3 + TypeScript + Pinia + Element Plus + ECharts + Vite，采用组合式 API 与函数式模式。
- 后端：Java 21 + Spring Boot + JPA + WebSocket + Redis + Kafka（MVP 使用内存桩件），结合虚拟线程与并发工具。
- AI 服务：Python + FastAPI + LangChain + LangGraph + WebSocket 传输，不得直接执行 SQL。
- 跨服务通信使用结构化 JSON 模式，存放于 `specs/<feature>/contracts/`。
...
本仓库的所有文档均集中存放于 `docs/` 目录，并统一使用简体中文撰写；对外回答也使用中文。若出现专业术语，可在中文后括注英文原词以降低歧义（如：虚拟线程（virtual threads））。

## 文档结构

- `docs/constitution.md` — 项目宪章（含文档规约）
- `docs/specs/001-business-system/plan.md` — 业务系统实施计划（当前优先）
- `docs/specs/001-business-system/spec.md` — 业务系统需求规格
- `docs/specs/001-business-system/tasks.md` — 业务系统任务清单
- `docs/specs/001-business-system/contracts/README.md` — 业务系统契约说明
- `docs/specs/002-intelligent-querying/plan.md` — 智能问数实施计划（后续阶段）
- `docs/specs/002-intelligent-querying/spec.md` — 智能问数需求规格
- `docs/specs/002-intelligent-querying/tasks.md` — 智能问数任务清单
- `docs/specs/002-intelligent-querying/contracts/README.md` — 智能问数契约说明

## 维护说明

- 新增或修改文档时，请直接在 `docs/` 下进行；不再在仓库其他目录存放文档。
...
来源：`docs/specs/001-business-system/spec.md`

---

（见 `docs/specs/001-business-system/spec.md`，内容摘要：角色权限、主数据、用例、API 草案、数据模型、审计与合规、非功能与错误码）

## 4. 功能 001 进销存业务系统——实施计划

来源：`docs/specs/001-business-system/plan.md`

---

（见 `docs/specs/001-business-system/plan.md`，内容摘要：目标与范围、里程碑（M1~M6）、风险与验收）

## 5. 功能 001 进销存业务系统——任务清单

来源：`docs/specs/001-business-system/tasks.md`

---

（见 `docs/specs/001-business-system/tasks.md`，内容摘要：阶段任务与“完成后同步记录”规则）

## 6. 功能 001 进销存业务系统——契约说明

来源：`docs/specs/001-business-system/contracts/README.md`

---

（见 `docs/specs/001-business-system/contracts/README.md`，内容摘要：REST/领域对象契约、版本与生成）
...
来源：`docs/specs/002-intelligent-querying/spec.md`

---

## 8. 功能 002 智能分析问询系统——实施计划

来源：`docs/specs/002-intelligent-querying/plan.md`

---

（见 `docs/specs/002-intelligent-querying/plan.md`）

## 9. 功能 002 智能分析问询系统——任务清单

来源：`docs/specs/002-intelligent-querying/tasks.md`

---

（见 `docs/specs/002-intelligent-querying/tasks.md`）

## 10. 功能 002 智能分析问询系统——契约说明

来源：`docs/specs/002-intelligent-querying/contracts/README.md`

---

（见 `docs/specs/002-intelligent-querying/contracts/README.md`）

在此存放 `RequestEnvelope`、`BusinessRequest`、`SQLRequest`、`InsightPayload` 以及 WebSocket 事件负载的 JSON Schema 与协议说明。请基于这些规范文档生成各语言的绑定（类型/模型）。
